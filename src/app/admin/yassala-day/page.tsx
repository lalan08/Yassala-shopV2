...
const S = {
  ...
  btnPrimary: { background: "linear-gradient(135deg,#fbbf24,#f59e0b)", color: "#000", border: "none", borderRadius: 6, padding: "11px 24px", ...mono, fontSize: ".85rem", letterSpacing: ".08em", cursor: "pointer", fontWeight: 700 },
  ...
  btnDanger: { background: "transparent", border: "1px solid rgba(255,45,120,.25)", color: "#ff2d78", borderRadius: 6, padding: "8px 14px", ...mono, fontSize: ".78rem", cursor: "pointer" },
...
export default function ServiceDayPage() {
  ...
  if (!auth) {
    return (
      <div style={{ ...S.page, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", ...mono, color: "#5a5470", fontSize: ".85rem" }}>
  ...
  const dayStats = [
    { label: "Catégories", emoji: "📦", count: catCount },
    { label: "Produits",   emoji: "🍔", count: prodCount },
    { label: "Packs",      emoji: "🎁", count: packCount },
    { label: "Offres",     emoji: "🏷️", count: offerCount },
  ];
  const nightStats = [
    { label: "Catégories", emoji: "📦", count: nightCatCount },
    { label: "Produits",   emoji: "🍔", count: nightProdCount },
    { label: "Packs",      emoji: "🎁", count: nightPackCount },
    { label: "Offres",     emoji: "🏷️", count: nightOfferCount },
  ];
  ...
  return (
    <div style={S.page}>
      ...
      <div style={S.topbar}>
        ...
        <span style={{ ...mono, fontSize: ".85rem", color: "#fbbf24", letterSpacing: ".1em", fontWeight: 700 }}>
          ☀️ SERVICE DAY
        </span>
        <button
          onClick={() => { setForm(blankForm); setEditId(null); setShowForm(v => !v); }}
      ...
      <div style={S.content}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontWeight: 800, fontSize: "1.6rem", margin: 0, marginBottom: 4 }}>
            ☀️ <span style={{ color: "#fbbf24" }}>SERVICE DAY — ÉTABLISSEMENTS</span>
          </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 20 }}>
          {dayStats.map(s => (
            <div key={s.label} style={{ background: "#0c0918", border: "1px solid rgba(251,191,36,.18)", borderRadius: 10, padding: "18px 16px 14px" }}>
              <div style={{ fontSize: "1.6rem", marginBottom: 6 }}>{s.emoji}</div>
              <div style={{ fontWeight: 700, fontSize: ".9rem" }}>{s.label} <span style={{ color: "#fbbf24", fontSize: ".75rem", ...mono }}>DAY</span></div>
              <div style={{ ...mono, fontSize: ".78rem", color: "#5a5470", marginTop: 4 }}>{s.count} éléments</div>
            </div>
          ))}
        </div>
        ...
        <div style={{
          ...
          border: "1px solid rgba(167,139,250,.15)", borderRadius: 10, padding: "18px 20px", marginBottom: 28,
          display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" as const, gap: 12,
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ fontSize: "1.2rem" }}>🌙</span>
            <span style={{ ...mono, fontSize: ".78rem", color: "#a78bfa", letterSpacing: ".1em" }}>SERVICE NIGHT</span>
          </div>
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap" as const }}>
            {nightStats.map(s => (
              <div key={s.label} style={{ textAlign: "center" as const }}>
                <div style={{ ...mono, fontSize: ".72rem", color: "#6b5fa0" }}>{s.label}</div>
                <div style={{ fontWeight: 700, color: "#a78bfa", fontSize: ".95rem" }}>{s.count}</div>
              </div>
            ))}
          </div>
          <a href="/admin/yassala-night" style={{ ...mono, fontSize: ".75rem", color: "#a78bfa", textDecoration: "none" }}>
            Gérer NIGHT →
          </a>
        </div>
            ...
            <div style={{ display: "grid", gap: 16 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                ...
                <div>
                  ...
                  <input value={form.slug} onChange={e => setForm(f => ({ ...f, slug: e.target.value }))} placeholder="boulangerie-soleil" style={{ ...S.input, ...mono }} />
                </div>
              </div>
              <div>
              ...
              {/* Images */}
              <div style={{ ...mono, fontSize: ".7rem", color: "#fbbf24", letterSpacing: ".1em", marginBottom: -4 }}>// IMAGES</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, padding: "16px 18px", background: "rgba(251,191,36,.04)", border: "1px solid rgba(251,191,36,.12)", borderRadius: 10 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 6 }}>
                  ...
                  <span onClick={() => !uploading && logoInputRef.current?.click()} style={{ ...mono, fontSize: ".62rem", color: "#5a5470", cursor: "pointer", textAlign: "center" as const }}>
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, alignItems: "center", gap: 6 }}>
                  ...
                  <div onClick={() => !uploadingCover && coverInputRef.current?.click()}
                    style={{ width: "100%", height: 80, borderRadius: 10, border: `2px dashed ${form.coverUrl ? "rgba(251,191,36,.45)" : "rgba(255,255,255,.18)"}`, background: form.coverUrl ? "#000" : form.bgColor || "rgba(255,255,255,.03)", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", overflow: "hidden", opacity: uploadingCover ? 0.6 : 1 }}>
                  ...
                  <span onClick={() => !uploadingCover && coverInputRef.current?.click()} style={{ ...mono, fontSize: ".62rem", color: "#5a5470", cursor: "pointer", textAlign: "center" as const }}>
              ...
              {/* Affichage carte */}
              <div style={{ ...mono, fontSize: ".7rem", color: "#fbbf24", letterSpacing: ".1em", marginBottom: -4 }}>// AFFICHAGE CARTE</div>
              <div style={{ display: "grid", gap: 14, padding: "16px 18px", background: "rgba(255,45,120,.03)", border: "1px solid rgba(255,45,120,.12)", borderRadius: 10 }}>
                ...
                <div>
                  <label style={S.label}>BADGE PROMO</label>
                  <input value={form.promo} onChange={e => setForm(f => ({ ...f, promo: e.target.value }))} placeholder="Ex: -10€ dès 35€" style={S.input} />
                </div>
              </div>

              {/* Livraison & Note */}
              <div style={{ ...mono, fontSize: ".7rem", color: "#fbbf24", letterSpacing: ".1em", marginBottom: -4 }}>// LIVRAISON & NOTE</div>
              ...
              {/* Statuts */}
              <div style={{ ...mono, fontSize: ".7rem", color: "#fbbf24", letterSpacing: ".1em", marginBottom: -4 }}>// STATUTS</div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, padding: "16px 18px", background: "rgba(255,255,255,.02)", border: "1px solid rgba(255,255,255,.07)", borderRadius: 10 }}>
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ ...mono, fontSize: ".78rem", color: form.isActive ? "#fbbf24" : "#5a5470" }}>{form.isActive ? "ACTIF" : "INACTIF"}</span>
                  </div>
                </div>
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ ...mono, fontSize: ".78rem", color: form.isOpen ? "#22c55e" : "#5a5470" }}>{form.isOpen ? "OUVERT" : "FERMÉ"}</span>
                  </div>
                  {!form.isOpen && (
                ...
                <div style={{ display: "flex", flexDirection: "column" as const, gap: 8 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <span style={{ ...mono, fontSize: ".78rem", color: form.isComingSoon ? "#a855f7" : "#5a5470" }}>{form.isComingSoon ? "BIENTÔT" : "NORMAL"}</span>
                  </div>
                </div>
              </div>

              {/* Code d'accès */}
              <div style={{ ...mono, fontSize: ".7rem", color: "#fbbf24", letterSpacing: ".1em", marginBottom: -4 }}>// ACCÈS ESPACE PRO</div>
              <div style={{ padding: "16px 18px", background: "rgba(251,191,36,.04)", border: "1px solid rgba(251,191,36,.15)", borderRadius: 10 }}>
                ...
                <input
                  ...
                  style={S.input}
                />
                <div style={{ ...mono, fontSize: ".62rem", color: "#5a5470", marginTop: 8 }}>
        ...
        {etablissements.length === 0 ? (
          <div style={{ textAlign: "center", padding: "64px 0", border: "1px dashed rgba(251,191,36,.15)", borderRadius: 12 }}>
            ...
            <div style={{ ...mono, fontSize: ".85rem", color: "#5a5470", marginBottom: 16 }}>
            ...
            {etablissements.map(etab => (
              <div key={etab.id} style={{ background: "rgba(255,255,255,.02)", border: `1px solid ${etab.isActive ? "rgba(251,191,36,.18)" : "rgba(255,255,255,.05)"}`, borderRadius: 12, padding: "16px 22px", display: "flex", alignItems: "center", gap: 18, opacity: etab.isActive ? 1 : 0.65, transition: "all .15s" }}>
                ...
                <div title="Cliquer pour changer le logo" onClick={() => { setQuickLogoEtabId(etab.id!); setTimeout(() => quickLogoRef.current?.click(), 10); }}
                  style={{ width: 52, height: 52, borderRadius: 10, background: "rgba(251,191,36,.08)", border: "1px solid rgba(251,191,36,.15)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.6rem", flexShrink: 0, overflow: "hidden", cursor: "pointer" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ ...mono, fontSize: ".7rem", color: "#4b4468", marginTop: 4, display: "flex", gap: 14, flexWrap: "wrap" as const }}>
                ...
                <div style={{ display: "flex", gap: 8, flexShrink: 0, flexWrap: "wrap" as const }}>
                  ...
                  <button onClick={() => startEdit(etab)}
                    style={{ background: "transparent", border: "1px solid rgba(251,191,36,.3)", color: "#fbbf24", padding: "8px 14px", borderRadius: 6, ...mono, fontSize: ".75rem", cursor: "pointer" }}>
                    ✏️ MODIFIER
                  </button>
                  <a href={`/admin/yassala-day/etablissements/${etab.id}`}
                    style={{ background: "linear-gradient(135deg,#fbbf24,#f59e0b)", color: "#000", padding: "8px 16px", borderRadius: 6, ...mono, fontSize: ".75rem", fontWeight: 700, textDecoration: "none", display: "inline-flex", alignItems: "center" }}>
                    📋 GÉRER LE MENU
                  </a>
