...
type Etab = { id?: string; name: string; logoUrl?: string; isOpen?: boolean };
...
const inp = (extra?: object) => ({
  width: "100%", background: "#08050f", border: `1px solid ${BORDER_DIM}`,
...
export default function EtablissementMenuPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      setAuth(parsed);
    } catch { router.replace("/etablissement/login"); }
  }, [etabId, router]);
  ...
  useEffect(() => {
    ...
    const u1 = onSnapshot(byEtab("day_categories"), snap =>
      setCats(snap.docs.map(d => ({ id: d.id, ...d.data() } as Cat)).sort((a, b) => (a.order ?? 0) - (b.order ?? 0)))
    );
    const u2 = onSnapshot(byEtab("day_products"), snap =>
  ...
  const saveCat = async () => {
    ...
    if (editCat?.id) {
      await updateDoc(doc(db, "day_categories", editCat.id), { ...f, key, etablissementId: etabId });
      setEditCat(null);
      showMsg("Catégorie mise à jour ✓");
    } else {
      await addDoc(collection(db, "day_categories"), { ...f, key, order: cats.length, etablissementId: etabId });
  ...
  const toggleProd = async (p: Prod & { id?: string }) => {
    if (!p.id) return;
    await updateDoc(doc(db, "day_products", p.id), { isActive: p.isActive === false });
  };
    ...
    <div style={{ minHeight: "100vh", background: BG, color: "#f0eeff", fontFamily: "'Inter',sans-serif" }}>
      ...
      <div style={{
        ...
        <div style={{ marginLeft: "auto" }}>
          <button
            onClick={() => { localStorage.removeItem("yassala_etab_auth"); router.replace("/etablissement/login"); }}
            style={{ ...MONO, fontSize: ".72rem", color: "#5a5470", background: "transparent", border: `1px solid ${BORDER_DIM}`, borderRadius: 6, padding: "6px 12px", cursor: "pointer" }}>
            DÉCONNEXION
          </button>
        </div>
      </div>
        ...
        <div style={{
          ...
          {cats.length === 0 && (
            <div style={{
              ...
              <span style={{ color: "#5a4468" }}>(ex: 🍔 Burgers, 🥤 Boissons)</span><br />
              ...
              return (
                <div key={p.id}
                  ...
                  <div style={{ height: 125, background: "rgba(255,45,120,.04)", position: "relative", overflow: "hidden" }}>
                    ...
                    {p.badge && (
                      <span style={{ position: "absolute", top: 7, left: 7, background: PINK, color: "#fff", padding: "2px 8px", borderRadius: 4, ...MONO, fontSize: ".62rem" }}>
                        {p.badge}
                      </span>
                    )}
                    {p.isActive === false && (
      ...
      {drawer && (
        <div
          ...
          <div style={{
            width: "100%", maxWidth: 440, background: "#0e0a1a",
            ...
            <div style={{ padding: "18px 22px", display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="Ex: Sandwich poulet grillé"
                  style={inp()}
                />
              </div>
              ...
              <div
                ...
                onClick={() => setDrawer(d => d && ({ ...d, isActive: d.isActive === false }))}>
                <div style={{
