...
export default function AdminPage() {
  ...
  const [tab, setTab]             = useState<"dashboard"|"products"|"categories"|"packs"|"orders"|"settings"|"banners"|"coupons">("dashboard");
        ...
        <main className="admin-main" style={{flex:1,padding:"28px",overflowY:"auto",animation:"fadeUp .3s both"}}>
          ...
          {tab === "settings" && (
            <div>
              ...
              {/* ── Bannière promo catalogue ── */}
              <div className="admin-settings-form" style={{background:"#0c0918",border:"1px solid rgba(255,255,255,.06)",
                borderRadius:8,padding:24,maxWidth:500,display:"grid",gap:14,marginTop:24}}>
                <div style={{fontFamily:"'Black Ops One',cursive",fontSize:"1rem",letterSpacing:".06em"}}>
                  🎉 <span style={{color:"#ffb400"}}>BANNIÈRE PROMO CATALOGUE</span>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:12}}>
                  <span style={{fontFamily:"'Share Tech Mono',monospace",fontSize:".8rem",color:"#888",letterSpacing:".08em"}}>ACTIVE</span>
                  <button onClick={() => setSettings(s => ({...s, promoBanner:{text:"",emoji:"🎉",color:"pink",...s.promoBanner,active:!(s.promoBanner?.active??false)}}))}
                    style={{background: settings.promoBanner?.active ? "rgba(184,255,0,.15)" : "rgba(255,255,255,.05)",
                      border: settings.promoBanner?.active ? "1px solid #b8ff00" : "1px solid rgba(255,255,255,.15)",
                      color: settings.promoBanner?.active ? "#b8ff00" : "#666",
                      padding:"5px 14px",borderRadius:12,fontFamily:"'Share Tech Mono',monospace",
                      fontSize:".78rem",letterSpacing:".08em",cursor:"pointer"}}>
                    {settings.promoBanner?.active ? "ON" : "OFF"}
                  </button>
                </div>
                <div style={{display:"grid",gap:8}}>
                  <label style={{fontFamily:"'Share Tech Mono',monospace",fontSize:".72rem",color:"#888",letterSpacing:".08em",textTransform:"uppercase"}}>Emoji</label>
                  <input value={settings.promoBanner?.emoji ?? "🎉"}
                    onChange={e => setSettings(s => ({...s, promoBanner:{text:"",emoji:"🎉",active:false,color:"pink",...s.promoBanner,emoji:e.target.value}}))}
                    placeholder="🎉" maxLength={4}
                    style={{background:"#080514",border:"1px solid rgba(255,255,255,.1)",borderRadius:4,
                      padding:"10px 14px",color:"#e0d9ff",fontFamily:"'Share Tech Mono',monospace",fontSize:".85rem",outline:"none",width:80}} />
                </div>
                <div style={{display:"grid",gap:8}}>
                  <label style={{fontFamily:"'Share Tech Mono',monospace",fontSize:".72rem",color:"#888",letterSpacing:".08em",textTransform:"uppercase"}}>Texte du message</label>
                  <input value={settings.promoBanner?.text ?? ""}
                    onChange={e => setSettings(s => ({...s, promoBanner:{text:"",emoji:"🎉",active:false,color:"pink",...s.promoBanner,text:e.target.value}}))}
                    placeholder="Livraison offerte ce soir dès 30€ !"
                    style={{background:"#080514",border:"1px solid rgba(255,255,255,.1)",borderRadius:4,
                      padding:"10px 14px",color:"#e0d9ff",fontFamily:"'Share Tech Mono',monospace",fontSize:".85rem",outline:"none"}} />
                </div>
                <div style={{display:"grid",gap:8}}>
                  <label style={{fontFamily:"'Share Tech Mono',monospace",fontSize:".72rem",color:"#888",letterSpacing:".08em",textTransform:"uppercase"}}>Couleur</label>
                  <div style={{display:"flex",gap:8}}>
                    {(["pink","cyan","green","gold"] as const).map(c => (
                      <button key={c} onClick={() => setSettings(s => ({...s, promoBanner:{text:"",emoji:"🎉",active:false,...s.promoBanner,color:c}}))}
                        style={{padding:"6px 14px",borderRadius:12,cursor:"pointer",
                          fontFamily:"'Share Tech Mono',monospace",fontSize:".72rem",letterSpacing:".08em",
                          background: settings.promoBanner?.color===c
                            ? c==="pink" ? "rgba(255,45,120,.25)" : c==="cyan" ? "rgba(0,245,255,.2)" : c==="green" ? "rgba(184,255,0,.2)" : "rgba(255,180,0,.2)"
                            : "rgba(255,255,255,.04)",
                          border: settings.promoBanner?.color===c
                            ? c==="pink" ? "1px solid #ff2d78" : c==="cyan" ? "1px solid #00f5ff" : c==="green" ? "1px solid #b8ff00" : "1px solid #ffb400"
                            : "1px solid rgba(255,255,255,.12)",
                          color: settings.promoBanner?.color===c
                            ? c==="pink" ? "#ff2d78" : c==="cyan" ? "#00f5ff" : c==="green" ? "#b8ff00" : "#ffb400"
                            : "#666"}}>
                        {c}
                      </button>
                    ))}
                ...
                <button onClick={saveSettings}
                  style={{background:"#ffb400",color:"#000",border:"none",borderRadius:4,
                    padding:"11px",fontFamily:"'Rajdhani',sans-serif",fontWeight:700,fontSize:".9rem",
                    letterSpacing:".08em",textTransform:"uppercase",cursor:"pointer",marginTop:4}}>
